...
static int tcp_disordered_ack(const struct sock *sk, const struct sk_buff *skb)
{
	...
	return (/* 1. Pure ACK with correct sequence number. */
		...
		!tcp_may_update_window(meta_tp, data_ack, data_seq, ntohs(th->window) << tp->rx_opt.snd_wscale) &&
...
static void tcp_ofo_queue(struct sock *sk)
{
	...
	while ((skb = skb_peek(&tp->out_of_order_queue)) != NULL) {
		...
		__skb_unlink(skb, &tp->out_of_order_queue);
		new_mapping = (tp->mpc) ? mtcp_get_dataseq_mapping(tp, skb) : 0;
...
static void check_buffers(struct multipath_pcb *mpcb)
{
	...
	mtcp_for_each_sk(mpcb, sk, tp) {
		...
		for(skb = skb_peek(&tp->out_of_order_queue);
		    skb;
		    skb = (skb_queue_is_last(&tp->out_of_order_queue, skb) ?
			    NULL :
...
static inline int tcp_try_rmem_schedule(struct sock *sk, unsigned int size)
{
	...
	if (tp->mpc) {
		...
		if (atomic_read(&meta_sk->sk_rmem_alloc) >
			...
			mtcp_debug("%s: mpcb copied seq:%#x\n", __FUNCTION__,
					meta_tp->copied_seq);
...
queue_and_out:
		...
		if (eaten > 0)
			 ...
			 * mtcp_data_ready, we are already in the process
			 * context).
			 */
			if (tp->mpc) {
				if (mapping == 1)
...
static void tcp_new_space(struct sock *sk, struct sock *mpcb_sk)
{
	...
	if (tcp_should_expand_sndbuf(sk,mpcb_sk)) {
		...
		if (mpcb) {
			 ...
			 * is 2*max(delays).
			 */
			/* If cur_bw_est not yet computed, just delay the
			   re-evaluation of sndbuf */
			if (!tp->cur_bw_est)
				demanded = 0;
			else
					 ...
					 struct tcphdr *th, unsigned len)
{
	...
	if (th->ack) {
		...
		if (!is_master_tp(tp) &&
				tp->rx_opt.mtcp_recv_token != loc_token(mpcb))
			goto reset_and_undo;
