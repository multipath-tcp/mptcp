	      ...
	      int nonagle)
{
	...
	if (mtcp_next_segment(meta_sk, NULL)) {
		...
		size_t len, int nonblock, int flags, int *addr_len)
{
	...
	do {
		...
		if (mpcb) {
			 ...
			 * receive queue
			 */
			if (mtcp_check_rcv_queue(mpcb, msg, &len, seq,
						 &copied, flags) < 0) {
				printk(KERN_ERR "error in "
				       "mtcp_check_rcv_queue\n");
				/* Exception. Bailout! */
				if (!copied)
					copied = -EFAULT;
				break;
