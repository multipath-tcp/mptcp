...
/* Generic function to iterate over active and backup subflows and
					  ...
					  bool zero_wnd_test)
{
	...
	sk = get_subflow_from_selectors(mpcb, skb, &subflow_is_active,
					zero_wnd_test, &force);
	if (force)
		/* one unused active sk or one NULL sk when there are temporally
		 * unavailable sks is returned
		 */
		return sk;
	...
	sk = get_subflow_from_selectors(mpcb, skb, &subflow_is_backup,
					zero_wnd_test, &force);
	if (sk && !force)
		/* one used sk is going to be returned, so reset path-mask */
		TCP_SKB_CB(skb)->path_mask = 0;
	/* one used, unsed or NULL backup sk is returned */
	return sk;
}
